...
Below we present our model tuning and validation results. The former indicate the found optimal Regularization parameter $\lambda$ values. The latter provide the trained model `RMSE` scores on the `validation` set. We start with presenting the *Basic model* results and then continue with those of the *Timing model*.

## Basic model
```{r, echo=F}
bmr <- movielens_report$basic_model_res
```
Training the *Basic model* on the range of $\lambda$ in [`r min(bmr$model$lambdas)`, `r max(bmr$model$lambdas)`] has revealed that the optimal value is $\lambda^{b}_{opt} =$ `r bmr$model$lambda`, with the corresponding minimum `RMSE` score on the `testing` set `r min(bmr$model$rmses)`, see the training plot below:
```{r, echo = F}
...
```
Training the *Timing model* on the range of $\lambda$ in [`r min(tmr$model$lambdas)`, `r max(tmr$model$lambdas)`] has revealed that the optimal value is $\lambda^{b}_{opt} =$ `r tmr$model$lambda`, with the corresponding minimum `RMSE` score on the `testing` set `r min(tmr$model$rmses)`, see the training plot below:
```{r, echo = F}
