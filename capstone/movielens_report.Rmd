...
The actual *Base model* movie rating estimate for movie $u$ and user $u$ will then be:
\[
...
* $t_{m,u}$ -- is the time at which the movie $m$ is to be rated by the user $u$
    + We shall stratify our time per week, as was done to show the *(O.V)* effect
* $f\left(.\right)$ -- a smooth function of its argument $t_{m,u}$
    + We shall use the Local Polynomial Regression Fitting (*LOESS*) with the degree $2$ for this
    
As with the *Basic model* the coefficients are to be estimated. Moreover, we will also apply Regularization to the timing effect represented by $f\left(t_{m,u}\right)$ to account for the weeks with too few movie ratings, if any. The smooth function $f\left(.\right)$ will be computed as the polynomial surface based on the average movie rating per week, using the LOESS algorithm.

Let $t = t_{m,u}$, then the movie rating estimate of the *Timing model* for movie $m$ and user $u$ in week $t$ is:
\[
...
\]
\[
    \hat{b_{t}} = \frac{\sum_{u=1}^{N_{t}}\left(f\left(t_{m,u}\right) - \hat{\mu}\right)}{\lambda + N_{t}}
\]
\[
...
* $N_{t}$ - is the number of movie ratings given in week $t = t_{m,u}$

We do not provide any mathematical proof of the *Timing model* correctness, but we are going to share out intuition for it. Without Regularization, $\hat{b_{t}}$ is to capture the average rating deviation in week $t$ from the overall average rating. The deviation is to be a smooth function of time which we therefore estimate as $\hat{b_{t}} = f\left(t_{m,u}\right) - \hat{\mu}$. If we now add Regularization on top, we shall the get the $\hat{b_{t}}$ formula as above.

# Results
<!--a results section that presents the modeling results and discusses the model performance-->
In this section we present our model tuning and validation results. The former give insights into the model training and indicate the found optimal $\lambda$ values. The latter consist of the model `RMSE` score on the `validation` set. We start with the *Basic model* and then proceed with the *Timing model*. 

## Basic model
```{r, echo=F}
bmr <- movielens_report$basic_model_res
```
Training the *Basic model* on the range of $\lambda$ has revealed that the optimal value $\lambda^{b}_{opt} =$ `r bmr$model$lambda` with the corresponding minimum `RMSE` score on the `testing` set `r min(bmr$model$rmses)`. Below one can see the training plot for the model:
```{r, echo = F}
min_rmse <- min(bmr$model$rmses)
max_rmse <- max(bmr$model$rmses)
tibble(lambdas = bmr$model$lambdas,
       rmses = bmr$model$rmses) %>%
...
```
Training the *Timing model* on the range of $\lambda$ has revealed that the optimal value $\lambda^{b}_{opt} =$ `r tmr$model$lambda` with the corresponding minimum `RMSE` score on the `testing` set `r min(tmr$model$rmses)`. Below one can see the training plot for the model:
```{r, echo = F}
min_rmse <- min(tmr$model$rmses)
max_rmse <- max(tmr$model$rmses)
tibble(lambdas = tmr$model$lambdas,
       rmses = tmr$model$rmses) %>%
  ggplot(aes(lambdas, rmses)) +
  geom_point() +
  geom_hline(aes(yintercept = min_rmse, color=I("brown")), lty=2) +
  scale_y_continuous(breaks=seq(min_rmse, max_rmse, length.out = 10)) +
