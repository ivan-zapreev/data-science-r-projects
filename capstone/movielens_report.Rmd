...
Note that, for the Regularization with individual lambdas for the movie and user effects, the $\hat{b_{m}}$ and $\hat{b_{u}}$ formulae used in the *Basic* and *Timing* models will be accordingly altered as follows:
\[
...
bt_rmse <- min(bmr$model$tuning_grid$rmse)
```
Training the *Basic model* on the range of $\lambda$ in [`r lm_min`, `r lm_max`] has revealed that the optimal value is $\lambda_{opt} =$ `r bmr$model$lambda_m`, with the corresponding minimum `RMSE` score on the `testing` set `r bt_rmse`, see the training plot below:
```{r, echo = F}
...
bt_rmse <- min(bmilr$model$tuning_grid$rmse)
...
```
Training the *Basic model* on the range of $\left(\lambda^{m},\;\lambda^{u}\right)$ in [`r lm_min`, `r lm_max`]x[`r lu_min`, `r lu_max`] has revealed that the optimal values are $\lambda^{m}_{opt} =$ `r bmilr$model$lambda_m` and $\lambda^{u}_{opt} =$ `r bmilr$model$lambda_u`, with the corresponding minimum `RMSE` score on the `testing` set `r bt_rmse`.
...
As one can see the found optimal lambda values are not equal to each other and the `RMSE` score on the `testing` set (`r bt_rmse`) is lower than that for the original *Basic model* (`r min(bmr$model$tuning_grid$rmse)`).
...
bt_rmse <- min(tmr$model$tuning_grid$rmse)
```
Training the *Timing model* on the range of $\lambda$ in [`r lm_min`, `r lm_max`] has revealed that the optimal value is $\lambda_{opt} =$ `r tmr$model$lambda_m`, with the corresponding minimum `RMSE` score on the `testing` set `r bt_rmse`, see the training plot below:
```{r, echo = F}
...
bt_rmse <- min(tmilr$model$tuning_grid$rmse)
```
Training the *Timing model* on the range of $\left(\lambda^{m},\;\lambda^{u}\right)$ in [`r lm_min`, `r lm_max`]x[`r lu_min`, `r lu_max`] has revealed that the optimal values are $\lambda^{m}_{opt} =$ `r tmilr$model$lambda_m` and $\lambda^{u}_{opt} =$ `r tmilr$model$lambda_u`, with the corresponding minimum `RMSE` score on the `testing` set `r bt_rmse`.
...
As one can see the found optimal lambda values are not equal to each other and the `RMSE` score on the `testing` set (`r bt_rmse`) is lower than that for the original *Timing model* (`r min(tmr$model$tuning_grid$rmse)`).
