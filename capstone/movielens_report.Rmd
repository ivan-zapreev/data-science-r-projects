...
Let $y_{u,i}$ be the rating for movie $i$ by user $u$ and $\hat{y}_{u,i}$ be our prediction for that rating, then: 

\[
  RMSE=\sqrt{\sum_{i=1}^{N}(y_{u,i}-\hat{y}_{u,i})^2}
\]
...
3. The `movielens` data frame is randomly split into two parts:
    2. `edx` -- a set storing the `90%` of the observations from the `movielens`, to be used for training
    1. `validation` -- a set storing the `10%` of the observations from the `movielens`, to be used for validation
