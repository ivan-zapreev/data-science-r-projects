...
* **Data observation**: About $99.3$% data variability is in the first two principle components;
...
The algorithms above will be run with the default parameters, unless tuning grid parameters are required. Therefore we will consider the entire `modeling` set, as any cross-validation will be done within the `train(.)` method of the `caret` package. Last but not least, we will only use the first two principle components of the available predictors data.

The model training will be done by the next function:
```{r, eval = FALSE}
  #--------------------------------------------------------------------
  #
  #--------------------------------------------------------------------
  train_model <- function(predictors_mtx, totalRent, method, ...) {
    #Train and return the model, with passing
    #through additional method parameters
    return(train(predictors_mtx,
                 totalRent,
                 method = method,
                 ...))
  }
...
The model validation will be done by the next function:
```{r, eval = FALSE}
  #--------------------------------------------------------------------
  #
  #--------------------------------------------------------------------
  evaluate_model <- function(fit_model, predictors_mtx, totalRent) {
    #Predict the raw data based in the fit model and predictor values
    totalRent_hat <- predict(knn_fit, predictors_mtx, type = "raw")

    #Compute and return the RMSE score
    return(RMSE(totalRent_hat, totalRent))
  }
```

The predictors matrix will be prepared by the next function:
```{r, eval = FALSE}
  #--------------------------------------------------------------------
  #
  #--------------------------------------------------------------------
  prepare_predictors <- function(pca_result, data_set, num_pc) {
    #Compute the full matrix from the data set by:
    # 1. Removing the totalRent column
    # 2. Converting to numeric matrix
    full_mtx <- data_set %>%
      select(-totalRent) %>%
      data.matrix(.)
    
    #Zero-center the columns, and then rotate
    rot_mtx <- sweep(full_mtx, 2, colMeans(val_mtx)) %*% pca_result$rotation

    #Only return the required principle component columns
    return(rot_mtx[,1:num_pc])
  }
```

Any models that will fail to be trained within $1$ hour on the hardware at hand:
```
...
```
will be discarded from further user.

All the additional model parameters, if required, will be presented in the *"Results"* section of this report.
