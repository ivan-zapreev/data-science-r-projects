...
```{r, echo = FALSE, warning= FALSE}
  ...
  plot_residuals(exp_res, lm_resid, "lm", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL, NUM_PLOT_SAMPLES)
```
The model is often off by about $50$--$100$ Euro in the estimates.

According to the Residual Standard Error, the actual `totalRent` can deviate from the regression plane by $\approx 236.7$, on average. In other words, given that the intercept (average `totalRent`) is $801.2$ Euro, we can say that the percentage error is (any prediction would still be off by) `r round(236.7 * 100 / 801.2,1)`%.
...
Finally, the `lm` model `validation` set `RMSE` is `r arog_report$lm_results$eval_res$rmse`, and it took just `r dseconds(train_dur_sec(arog_report$lm_results))` to train the model.
...
```
This model is also often off by about $50$--$100$ Euro in the estimates. The residuals are distributed very similarly as in the case of the `lm` model.

The one order of magnitude difference between the `Null` and `residual` deviances indicates a significant effect of the predictors on the model accuracy.

Finally, the `glm` model `validation` set `RMSE` is `r arog_report$glm_results$eval_res$rmse`, and it took just `r dseconds(train_dur_sec(arog_report$glm_results))` to train the model.
