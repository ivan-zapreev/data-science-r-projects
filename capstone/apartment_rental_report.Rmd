...
train_dur_sec <- function(mdl_res) {
  ...
  return(round(as.numeric(end_time - start_time)))
}
```
...
summary(lm_fit)
```
The residuals indicate that the fit is not ideal, as confirmed by the plot:
...
```
The 'lm' model is on average by `r round(mean(abs(lm_resid)))` Euro off in its price predictions:
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
...
Finally, the `lm` model `validation` set `RMSE` is `r round(arog_report$lm_results$eval_res$rmse)`, and it took just `r dseconds(train_dur_sec(arog_report$lm_results))` seconds to train the model.
...
summary(glm_fit)
```
Like for `lm`, the residuals indicate that the fit is not ideal, see the plot below:
...
```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  ...
  plot_residuals(exp_res, glm_resid, "glm", 
                 ...
                 sample_size = NUM_PLOT_SAMPLES)
```
The 'glm' model is on average by `r round(mean(abs(glm_resid)))` Euro off in its price predictions:
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
...
Finally, the `glm` model `validation` set `RMSE` is `r round(arog_report$glm_results$eval_res$rmse)`, and it took just `r dseconds(train_dur_sec(arog_report$glm_results))` seconds to train the model.
