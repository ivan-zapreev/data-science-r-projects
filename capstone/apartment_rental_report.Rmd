...
```{r, echo=F}
#This R chunk only contains helper functions
...
#--------------------------------------------------------------------
#Helper log-10-scale transformation functions
#--------------------------------------------------------------------
log_10_trans <- function(x) {return(sign(x)*log10(abs(x)))}
log_10_inv <- function(x) {return(sign(x)*10^abs(x))}
base_breaks <- function(n = 20){
  function(x) {
      axisTicks(log_10_trans(range(x, na.rm = TRUE)), log = TRUE, n = n)
  }
}

#--------------------------------------------------------------------
# This function is used to plot the residuals for the totalRent
#--------------------------------------------------------------------
plot_residuals <- function(exp_res, resid, model,
                           min_x = min(exp_res), max_x = max(exp_res),
                           min_y = min(resid), max_y = max(resid),
                           sample_size = length(exp_res)) {
  set.seed(1) #To be able to have equal plots for equal data
  data.frame(exp_res = exp_res, resid = resid) %>%
    arrange(exp_res) %>%
    sample_n(sample_size) %>%
    ggplot(aes(x=exp_res, y=resid)) + 
    scale_x_log10(limits = c(min_x, max_x)) +
    scale_y_continuous(limits = c(min_y, max_y),
      trans = trans_new( name = "log10",
                         transform = log_10_trans,
                         inverse = log_10_inv),
      breaks = base_breaks()) +
    geom_point(alpha=0.2) +
    labs(x="totalRent values (ordered)", y="totalRent residuals") +
    ggtitle(paste(paste("The totalRent residual error for ", model), " model")) +
    theme(plot.title = element_text(hjust = 0.5, face="bold"))
}

#--------------------------------------------------------------------
# This function is used to plot the residuals distribution for the totalRent
#--------------------------------------------------------------------
plot_residuals_dist <- function(resid, model,
                           min_x = min(resid), max_x = max(resid),
                           sample_size = length(resid)) {
  data.frame(resid = resid) %>%
    ggplot(aes(x = resid)) +
    scale_y_log10() +
    scale_x_continuous(limits = c(round(min_x), round(max_x)),
      trans = trans_new( name = "log10",
                         transform = log_10_trans,
                         inverse = log_10_inv),
      breaks = base_breaks()) +
    geom_histogram(binwidth = 0.125, colour = "black", fill = "lightblue") +
    labs(x="Residual", y="Count") +
    ggtitle(paste(paste("The totalRent residual error distribution for ", model), " model")) +
    theme(plot.title = element_text(hjust = 0.5, face="bold"))
}

#--------------------------------------------------------------------
# This function is used to plot the abs(residual) distribution for the totalRent
#--------------------------------------------------------------------
plot_abs_residuals_dist <- function(resid_abs, model,
                           min_x = min(resid_abs), max_x = max(resid_abs),
                           sample_size = length(resid_abs)) {
  data.frame(resid_abs = resid_abs) %>%
    ggplot(aes(x = resid_abs)) +
    scale_y_log10() +
    scale_x_continuous(limits = c(round(min_x), round(max_x))) +
    geom_histogram(binwidth = 10, colour = "black", fill = "lightblue") +
    labs(x="Residual", y="Count") +
    ggtitle(paste(paste("The abs(residual) distribution for ", model), " model")) +
    theme(plot.title = element_text(hjust = 0.5, face="bold"))
}

#--------------------------------------------------------------------
#Compute and return the duration in seconds of the model training
#--------------------------------------------------------------------
train_dur_sec <- function(mdl_res) {
  start_time <- mdl_res$train_res$start_time
  end_time <- mdl_res$train_res$end_time
  return(round(as.numeric(difftime(end_time, start_time,units="sec"))))
}
```
...
As stated on the [webpage](`r arog_report$AROG_DATA_SET_SITE_URL`) of the `r arog_report$AROG_DATA_SET_NAME`, it contains `r nrow(arog_data$raw_data)` rental offers scraped from the Germany's biggest real estate online platform [ImmobilienScout24](https://www.immobilienscout24.de/). 
...
4. **Present modeling results** -- see the  *"Modeling results"* section:
...
Below, we shall present our experimental results. First, we describe the hardware they were obtained on. Next, we we present the training and validation results for each individual model. Finally, we provide a brief summary of our findings.
...
```{r, echo = FALSE}
  ...
  exp_res <- arog_report$lm_results$train_res$fit_model$trainingData$.outcome
```

Please note that in this section when plotting the residual values, for the sake of better visualization, we will always take a `r NUM_PLOT_SAMPLES` sub-sample of the data, and limit the `totalRent` axis ($x$) range by [`r MIN_X_VAL`, `r MAX_X_VAL`], and the `residuals` axis ($y$) range by [`r MIN_Y_VAL`, `r MAX_Y_VAL`]. These settings do not influence the overall picture.

### Linear Regression -- `lm`
Consider the summary of the trained `lm` model: 
```{r, echo = FALSE}
lm_fit <- arog_report$lm_results$train_res$fit_model
summary(lm_fit)
```
The residuals are spread rather symmetrically around the zero indicating a good model fit:

```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  #Compute the residuals
  lm_resid <- resid(lm_fit)

  #Plot the residuals
  plot_residuals(exp_res, lm_resid, "lm", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL,
                 sample_size = NUM_PLOT_SAMPLES)
```
The histogram of the residual values looks as follows: 

```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
plot_residuals_dist(lm_resid, "lm",
                    MIN_Y_VAL, MAX_Y_VAL,
                    sample_size = NUM_PLOT_SAMPLES)
```
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
lm_resid_abs <- abs(lm_resid)
plot_abs_residuals_dist(lm_resid_abs, "lm", 0, 1000,
                    sample_size = NUM_PLOT_SAMPLES)
```

```{r, echo = F}
lm_resid_avg <- round(mean(lm_resid_abs), 2)
lm_resid_sd <- round(sd(lm_resid_abs), 2)
lm_rmse <- round(arog_report$lm_results$eval_res$rmse, 2)
lm_time <- dseconds(train_dur_sec(arog_report$lm_results))
```
Note that, the mean absolute residual value is `r lm_resid_avg` and the standard deviation is `r lm_resid_sd`.

According to the Residual Standard Error, the actual `totalRent` can deviate from the regression plane by $\approx 237$, on average. In other words, given that the intercept (average `totalRent`) is $\approx 801$ Euro, we can say that the percentage error is (any prediction would still be off by) `r round(236.7 * 100 / 801.2, 1)`%.
...
The `RMSE` computed on the `validation` set is `r lm_rmse`, and it took just `r lm_time` seconds to train this model.

### Generalized Linear Model -- `glm`
Consider the summary of the trained `glm` model: 
```{r, echo = FALSE}
glm_fit <- arog_report$glm_results$train_res$fit_model
summary(glm_fit)
```
The residuals are spread rather symmetrically around the zero indicating a good model fit:
...
```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  #Compute the residuals
  glm_resid <- resid(glm_fit)

  #Plot the residuals
  plot_residuals(exp_res, glm_resid, "glm", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL,
                 sample_size = NUM_PLOT_SAMPLES)
```
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
plot_residuals_dist(glm_resid, "glm",
                    MIN_Y_VAL, MAX_Y_VAL,
                    sample_size = NUM_PLOT_SAMPLES)
```
The absolute residual values are then distributed as follows:
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
glm_resid_abs <- abs(glm_resid)
plot_abs_residuals_dist(glm_resid_abs, "glm", 0, 1000,
                    sample_size = NUM_PLOT_SAMPLES)
```

```{r, echo = F}
glm_resid_avg <- round(mean(glm_resid_abs), 2)
glm_resid_sd <- round(sd(glm_resid_abs), 2)
glm_rmse <- round(arog_report$glm_results$eval_res$rmse, 2)
glm_time <- dseconds(train_dur_sec(arog_report$glm_results))
```
Note that, the mean absolute residual value is `r glm_resid_avg` and the standard deviation is `r glm_resid_sd`.
...
The `RMSE` computed on the `validation` set is `r glm_rmse`, and it took just `r glm_time` seconds to train this model.

### K-Nearest Neighbors -- `knn`
Consider the summary of the trained `knn` model: 
```{r, echo = FALSE}
knn_fit <- arog_report$knn_results$train_res$fit_model
summary(knn_fit)
```
...
plot(knn_fit, main="Tuning grid KNN performance")
```
The residuals are spread rather symmetrically around the zero indicating a good model fit:
...
```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  ...
  #Plot the residuals
  plot_residuals(exp_res, knn_resid, "knn", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL,
                 sample_size = NUM_PLOT_SAMPLES)
```
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
plot_residuals_dist(knn_resid, "knn",
                    MIN_Y_VAL, MAX_Y_VAL,
                    sample_size = NUM_PLOT_SAMPLES)
```
The absolute residual values are then distributed as follows:
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
knn_resid_abs <- abs(knn_resid)
plot_abs_residuals_dist(knn_resid_abs, "knn", 0, 1000,
                    sample_size = NUM_PLOT_SAMPLES)
```

```{r, echo = F}
knn_resid_avg <- round(mean(knn_resid_abs), 2)
knn_resid_sd <- round(sd(knn_resid_abs), 2)
knn_rmse <- round(arog_report$knn_results$eval_res$rmse, 2)
knn_time <- dseconds(train_dur_sec(arog_report$knn_results))
```
Note that, the mean absolute residual value is `r knn_resid_avg` and the standard deviation is `r knn_resid_sd`.

The `RMSE` computed on the `validation` set is `r knn_rmse`, and it took `r knn_time` seconds to train this model.

### Random Forest -- `Rborist`
Consider the summary of the trained `Rborist` model: 
```{r, echo = FALSE}
rb_fit <- arog_report$Rborist_results$train_res$fit_model
summary(rb_fit)
```
...
```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  #Compute the residuals
  rb_resid <- resid(rb_fit)

  #Plot the residuals
  plot_residuals(exp_res, rb_resid, "Rborist", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL,
                 sample_size = NUM_PLOT_SAMPLES)
```
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
plot_residuals_dist(rb_resid, "Rborist",
                    MIN_Y_VAL, MAX_Y_VAL,
                    sample_size = NUM_PLOT_SAMPLES)
```
The absolute residual values are then distributed as follows:
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
rb_resid_abs <- abs(rb_resid)
plot_abs_residuals_dist(rb_resid_abs, "Rborist", 0, 1000,
                    sample_size = NUM_PLOT_SAMPLES)
```

```{r, echo = F}
rb_resid_avg <- round(mean(rb_resid_abs), 2)
rb_resid_sd <- round(sd(rb_resid_abs), 2)
rb_rmse <- round(arog_report$Rborist_results$eval_res$rmse, 2)
rb_time <- dseconds(train_dur_sec(arog_report$Rborist_results))
```
Note that, the mean absolute residual value is `r rb_resid_avg` and the standard deviation is `r rb_resid_sd`.

The `RMSE` computed on the `validation` set is `r rb_rmse`, and it took `r rb_time` seconds to train this model.

### Support Vector Machines -- `svmLinear`
Consider the summary of the trained `svmLinear` model: 
```{r, echo = FALSE}
svm_fit <- arog_report$svmLinear_results$train_res$fit_model
summary(svm_fit)
```
...
svm_resid_avg <- NA
svm_resid_sd <- NA
svm_rmse <- round(arog_report$svmLinear_results$eval_res$rmse, 2)
svm_time <- dseconds(train_dur_sec(arog_report$svmLinear_results))
```

The `RMSE` computed on the `validation` set is `r svm_rmse`, and it took `r svm_time` seconds to train this model.
...
```{r, echo = FALSE}
gam_fit <- arog_report$gamLoess_results$train_res$fit_model
```
...
```{r, echo = FALSE, warning= FALSE, out.height = "35%", fig.align="center"}
  #Compute the residuals
  gam_resid <- resid(gam_fit)

  #Plot the residuals
  plot_residuals(exp_res, gam_resid, "gamLoess", 
                 MIN_X_VAL, MAX_X_VAL, 
                 MIN_Y_VAL, MAX_Y_VAL,
                 sample_size = NUM_PLOT_SAMPLES)
```
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
plot_residuals_dist(gam_resid, "gamLoess",
                    MIN_Y_VAL, MAX_Y_VAL,
                    sample_size = NUM_PLOT_SAMPLES)
```
The absolute residual values are then distributed as follows:
...
```{r, echo = FALSE, warning = FALSE, out.height = "35%", fig.align="center"}
gam_resid_abs <- abs(gam_resid)
plot_abs_residuals_dist(gam_resid_abs, "gamLoess", 0, 1000,
                    sample_size = NUM_PLOT_SAMPLES)
```

```{r, echo = F}
gam_resid_avg <- round(mean(gam_resid_abs), 2)
gam_resid_sd <- round(sd(gam_resid_abs), 2)
gam_rmse <- round(arog_report$gamLoess_results$eval_res$rmse, 2)
gam_time <- dseconds(train_dur_sec(arog_report$gamLoess_results))
```
Note that, the mean absolute residual value is `r gam_resid_avg` and the standard deviation is `r gam_resid_sd`.

The `RMSE` computed on the `validation` set is `r gam_rmse`, and it took `r gam_time` seconds to train this model.

## Results summary
First we shall summarize the training results for the models:
```{r, echo=FALSE}
  #Combine the main results from the trained models
  add_results <- function(old_res, new_res) {
    res <- new_res %>% 
      select(RMSE, Rsquared, MAE, RMSESD, RsquaredSD, MAESD) %>%
      rbind(old_res, .)
    return(res)
  }
  train_mdl_tbl <- add_results(tibble(), lm_fit$results)
  train_mdl_tbl <- add_results(train_mdl_tbl, glm_fit$results)
  knn_br_idx <- which(knn_fit$results$k == knn_fit$bestTune$k)
  knn_res <- slice(knn_fit$results, knn_br_idx)
  train_mdl_tbl <- add_results(train_mdl_tbl, knn_res)
  train_mdl_tbl <- add_results(train_mdl_tbl, rb_fit$results)
  train_mdl_tbl <- add_results(train_mdl_tbl, svm_fit$results)
  train_mdl_tbl <- add_results(train_mdl_tbl, gam_fit$results)
  
  #Add the 95% confidence interval for the Absolute Residual
  train_mdl_tbl <- train_mdl_tbl %>%
    mutate(ARMean = c(lm_resid_avg, glm_resid_avg, knn_resid_avg,
                            rb_resid_avg, svm_resid_avg, gam_resid_avg),
           ARSD = c(lm_resid_sd, glm_resid_sd, knn_resid_sd,
                     rb_resid_sd, svm_resid_sd, gam_resid_sd)) %>%
    round(., 2)
  
  #Set the row names
  rownames(train_mdl_tbl) <- c("lm", "glm", "knn", "Rborist", "svmLinear", "gamLoess")
  train_mdl_tbl <- train_mdl_tbl[c("RMSE", "RMSESD", "ARMean",  "ARSD",
                                   "Rsquared", "RsquaredSD", "MAE", "MAESD")]
  
  kable(train_mdl_tbl, caption = "Model Training results",
      col.names = c("Value", "SD", "Avg", "SD", "Value", "SD", "Value", "SD"),
      escape = FALSE, align=rep('c', 9)) %>%
  add_header_above(c("Model " = 1, "RMSE" = 2, "AR" = 2, "Rsquared" = 2 , "MAE" = 2)) %>%
  kable_styling(latex_options =c("striped", "hold_position"))
```
...
```{r, echo = FALSE}
  valid_mdl_tbl <- tibble(Model = c("lm", "glm", "knn",
                                    "Rborist", "svmLinear", "gamLoess"),
                          RMSE = c(lm_rmse, glm_rmse, knn_rmse,
                                   rb_rmse, svm_rmse, gam_rmse),
                          Time = c(lm_time, glm_time, knn_time,
                                   rb_time, svm_time, gam_time)) %>% arrange(RMSE)

  kable(valid_mdl_tbl, caption = "Model Validation results",
      col.names = c("Model", "RMSE", "Time (sec)"),
      escape = FALSE, align=rep('c', 3)) %>%
  add_header_above(c(" " = 1, "Summary" = 2)) %>%
  kable_styling(latex_options =c("striped", "hold_position"))
```

The clear top scoring models, with a marginal difference, are `svmLinear`, `lm`, `glm`, and `gamLoess.` Accounting for the training time, the clear winner is `lm`, with `glm` being very close behind. The other models (`Rborist`, and `knn`) performed significantly worse, both in `RMSE` and training times. None of the models was able to achieve the goal `TARGET_RMSE = ` `r TARGET_RMSE`. Yet, the `RMSE` of our top model is very close to that.

Let us also note that from analyzing the residuals, it seemed like `Rborist` should perform better in `RMSE` than other models. The validation results show that it is not the case. Its `RMSE` score of `r rb_rmse` is not impressive. We believe that the reason for that is the over fitting of the model on the training (`modeling`) set. Perhaps, this can be corrected by choosing the right tuning or control parameters for the model but this requires further investigation.
...
In this project we have taken upon an ambitious goal of building a prediction system for the apartment rental prices in Germany. To reach this goal we have employed supervised Machine Learning techniques studied within the [Data Science Professional program at HarvardX](https://www.edx.org/professional-certificate/harvardx-data-science). I particular, we have used Bayesian modeling, the `r arog_report$AROG_DATA_SET_NAME`, and Residual Mean Squared error (`RMSE`) for evaluating accuracy. The goal was to get a model that has an `RMSE` $\leq$ `TARGET_RMSE =` `r TARGET_RMSE`, which would mean that our model could predict the rent price within about $50$ Euro, on average.

The considered data set contains `r nrow(arog_data$raw_data)` rental offers scraped from the Germany's biggest real estate online platform [ImmobilienScout24](https://www.immobilienscout24.de/). It features important rental property attributes, such as living area size, rent, location, type of energy, and etc. The data scraping was done on three distinct dates: `r unique(arog_data$wrangled_data$date)` which are also part of the data.

We started our work with data wrangling which revealed that the data set has `r ncol(arog_data$raw_data)` data columns. This is a large number and therefore we have done an initial selection of `r ncol(arog_data$selected_data)` columns to be used in this case study. It turned out that the data at hand was also very "dirty". For instance, about $\frac{1}{2}$ of the columns had $2.5$ to $80$% of `N/A` values. Moreover, there were multiple inconsistencies present in the data such as, e.g., lack of coherence between the flat types (e.g. `"roof_storey"`) and the flat floors (e.g. $-1$). In addition, multiple columns had outlying values, e.g. flats at the `r max(arog_data$selected_data$floor, na.rm = TRUE)`'th floors and alike. All of these were handled by various techniques described in the *"Data wrangling"* section of this report. The cleaned and enriched data was then considered for further reshaping. For instance, the flat's location, originally split between multiple columns had to be recombined, and etc.

The resulting wrangled data turned out to be `r round(nrow(arog_data$wrangled_data)*100/nrow(arog_data$selected_data),2)`% of the original. We see it as a good result, considering the amount of initial data pollution. Further, the data was split into `modeling` (`r (1-arog_report$VALIDATION_TO_MODELING_SET_RATIO)*100`% of data) and `validation` (`r arog_report$VALIDATION_TO_MODELING_SET_RATIO*100`% of data) sets. The former was also split into `training` and `testing` sub-sets to be used for cross-validation but this turned out to be unnecessary[^16].

[^16]: Is explained in one of the subsequent paragraphs.

As the next step, we have performed data analyses. First, we checked the data at hand with respect to: 

* `timing` effects -- whether the scraping dates show any trends
* `location` effects -- whether the apartment location has a significant impact on rental prices

The former turned out to be not present, whereas the latter one was very vividly present. Then, due to the immense size of the number of predictors (`r ncol(arog_data$wrangled_data) - 1`), we have also performed *correlation analysis* of the predictors' matrix. From which we have observed multiple correlations between different data set columns. The subsequent *Principle Component Analysis* (`PCA`) has revealed that the first two principle components (`PC1` and `PC2`) explain $99.3$% of data variability. This convinced us to use the dimensionality reduction approach employing the `PCA` rotation matrix.

Since it has been shown that it suffices to use only `PC1` and `PC2` to explain most of the data variability, it has been decided NOT to build a new statistical model specifically for this case study, but rather to use the existing ones. The visualization of the total rent versus `PC1` and `PC2` has revealed a clear data clustering suitable for regression models. The modeling approach therefore was to: Take the common *regression models* available via the `caret` package: `lm`, `glm`, `knn`, `Rborist`, `svmLinear`, and `gamLoess`; and use them to predict the total rent prices based on `PC1` and `PC2`.

The model training times are often hard to predict. Therefore, we have decided to limit the training time per model by `r arog_report$TRAIN_CPU_TIME_OUT_SECONDS` CPU seconds. This turned out to be sufficient for all of the models at hand. Also, we did not use any specific model control or tuning parameters. The exception was `knn` for which we defined the `k` range. The results revealed that the `lm` model is the fastest (takes `r lm_time` seconds to train) and has the top performance (`RMSE` = `r lm_rmse`). All the other models, except for comparably performing `glm`; and slightly more accurate, but orders of magnitude slower `svmLinear`, had significantly higher $RMSE$s and longer training times. See the *"Results summary"* section for more details.

In this study, we have successfully performed all the steps needed to build a rental price prediction system. We did not reach the ambitious `TARGET_RMSE` score but we approached it very closely, even without selecting any special model control or tuning parameters.

## Future work
In the future one may consider extending this case study by comparing more different regression models, as well as tuning the used ones for better performance. Also, model ensembles can be taken to improve on the single model performances. One could also try using six first principle components for training the models, as those explain $99.99$% of predictors variability.
