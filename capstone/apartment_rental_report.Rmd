...
The resulting wrangled data turned out to be `round(nrow(arog_data$wrangled_data)*100/nrow(arog_data$selected_data),2)`% of the original. We see it as a good result, considering the amount of initial data pollution. Further, the data was split into `modeling` (`(1-arog_report$VALIDATION_TO_MODELING_SET_RATIO)*100`% of data) and `validation` (`r arog_report$VALIDATION_TO_MODELING_SET_RATIO*100`% of data) sets. The former was also split into `training` and `testing` sub-sets to be used for cross-validation but this turned out to be unnecessary[^16].
